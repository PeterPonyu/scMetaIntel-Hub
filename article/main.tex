...
corpus of 2{,}189 GEO studies and 171 evaluation queries. An 8B-parameter
model (llama3.1-8b-q4, composite score 0.776) ranks first, outperforming
14B--32B alternatives. Hybrid retrieval with organism filtering matches or
exceeds neural reranking at half the latency. Quantisation from 8-bit to
4-bit incurs negligible quality loss (mean $\Delta$composite $< 0.01$) while
yielding $1.4\times$ throughput gain. The corpus, queries, and all benchmark
code are publicly available.}

\keywords{single-cell genomics \and large language models \and metadata
  intelligence \and benchmark \and retrieval-augmented generation \and
  local inference}
...
\begin{enumerate}[leftmargin=2em, itemsep=2pt]
  ...
  \item An \textbf{empirically validated pipeline recipe}: hybrid retrieval
    with organism filtering outperforms neural reranking; an 8B model matches
    or exceeds larger alternatives; 4-bit quantisation incurs negligible
    quality loss.
...
performance for all 66 configurations. Composite scores range from 0.057 to
0.776 (spread\,=\,0.719), confirming strong discriminative power. The
top-ranked model is llama3.1-8b-q4 (composite 0.776), followed by
llama3.1-8b (0.768) and aya-expanse-8b (0.767). The top five includes two
Llama~3.1 variants, one Aya, one Qwen~3.5, and one Qwen~3 with thinking
mode (Table~\ref{tab:top5}).

\begin{table}[t]
\caption{Top-5 model configurations by composite score. Size is in billions
  of parameters; Tok/s measured on RTX~5090.}\label{tab:top5}
\begin{tabular*}{\textwidth}{@{\extracolsep\fill}clcccccc}
\toprule
Rank & Model & Family & Size & Parse & Cite Recall & Onto F1 & Composite \\
\midrule
1 & llama3.1-8b-q4   & Llama & 8B  & 0.967 & 0.883 & 0.760 & 0.776 \\
2 & llama3.1-8b      & Llama & 8B  & 0.833 & 0.933 & 0.813 & 0.768 \\
3 & aya-expanse-8b   & Aya   & 8B  & 1.000 & 0.950 & 0.688 & 0.767 \\
4 & qwen3.5-4b       & Qwen  & 4B  & 0.633 & 1.000 & 0.875 & 0.756 \\
5 & qwen3-1.7b+think & Qwen  & 1.7B & 0.700 & 0.917 & 0.781 & 0.755 \\
\bottomrule
\end{tabular*}
\end{table}

\textbf{Scale does not predict performance.} The principal finding is that
an 8B-parameter model achieves the highest composite score, surpassing 14B
and 32B alternatives. Among the top~10, model sizes range from 1.7B to
32B, with no monotonic size--quality relationship. The Spearman correlation
between parameter count and composite score across all 66 configurations is
weakly negative ($\rho = -0.08$), confirming that architectural design and
instruction tuning outweigh raw scale for these metadata-centric tasks
(Fig.~\ref{fig:scaling}).
...
Four pipeline configurations were compared end to end on 90 queries
...
the validated embedding and retrieval components. The quality-optimised
configuration (qwen3.5-27b) showed lower grounding (0.441), suggesting that
larger models in the answer-generation role are more prone to generating
text beyond the provided context.
...
(aya-expanse-8b, llama3.1-8b) excel at parsing and domain classification,
while models with deeper reasoning chains (qwen3-32b+think) lead on ontology
normalisation. Citation generation, the most practically important task,
...
that mid-size models (4--8B parameters) achieve optimal performance on
metadata intelligence tasks, hybrid retrieval with organism filtering
outperforms neural reranking at lower latency, and 4-bit quantisation
incurs negligible quality loss. We release the corpus of 2{,}189 enriched
